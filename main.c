...
Revised to include diffusivity calculations from other CEM code fragments by Andrew Ashton
    and to include variable wave climates by Chris Thomas 2013-2015.
...
---Chris Thomas (CWT) Changes Jan - Nov 2015---
...
3.  - Added variables to allow for changes in the wave climate through a run, initially by varying
      the Initial_Asym (A) and Initial_Highness (U) parameters, and for the calculation of diffusivities,
      and various fluxes over a static beach
        - Asym changed to Initial_Asym
        - Highness changed to Initial_Highness
        ...
        - Diff_Asym added, Diff_Highness added; these are the U and A parameters set for diffusivity calculations
        ...
        - ChangeWaveClimate added as a flag set in CEM_init.dat; 1 for yes, 0 for no.
        - DoDiff added as a flag set in CEM_init.dat; 1 for yes, 0 for no
        - DiffPause added as a logical to indicate when the program should undertake diffusivity calculations
        - nDiffSteps: the number of steps for diffusivity calculations; set in CEM_init.dat
        - DiffCalcStep: this sets the CurrentTimeStep at which the program should pause to do diffusivity calculations
        - StopAfterDiff: this is a logical to indicate whether the program should continue running after the diffusivity
            calculations. It is set in CEM_init.dat. If set to 0, then the program will continue in normal CEM mode,
            evolving the beach
        ...
        - Code changed to include the diffusivity calculations of Andrew and Brad as outlined in 2006b; control over
          these is by some 1/0 flags set in CEM_init.dat (see above and CEM_init.dat)
        - Code changed to save diffusivity wave climate data as this may differ from the wave climate used in the
          model to the point where diffusivity calculations start. File is DiffWaveData.dat
        - Code changed to set a flag to exclude the RockCalculations - these seem superfluous if working only on
          sandy coast models
        - Apologies for the lumpy code re diffusivity calcs vs normal CEM - utility over elegance and good programming
          practice...

        Chris Thomas Jan - Nov 2015
...
/*
    All the following time steps controls best set and read in the CEM_init.dat control file
    Initialised for the sake of completeness here
*/

int    ChangeWaveClimate = 0;   /* If set to 1 (best in CEM_init.dat) this allows for changing wave climate experiments */
int    RockCalcs = 0;           /* Only do rock calculations if required; RockCalcs = 1 for yes in CEM_init.dat */
int    DoDiff = 0;              /* Do diffusion calculations if set to 1; beach won't be evolved */
int    nDiffSteps = 0;          /* if DoDiff is 1, this sets the number of steps over which to undertake diffusivity calculations
...
/* Settings for Highness (1 - U) and Asymmetry (A) for wave climate properties */
...
double Diff_Asym = 0.0;         /* The Asymmetry value to be used for diffusivity calculations if DoDiff is trun (= 1) */
double Diff_Highness = 0.0;     /* The Highness value to be used for diffusivity calculations if DoDiff is true (= 1) */
int	   WaveIn = 0;		        /* Input Wave Distribution file: no = 0, binned file = 1, angle/period/height file =2 */

/* Initilisations and filenames */
...
char   savediffname[24] = "CEM_diff.dat";
...
char   DiffWaveData[24] = "DiffWaveData.dat"; /* This file contains the wave data used when calculating diffusivities */
...
/* Variables for the Qs and diffusivity (Gamma) calculations */

float   DiffSum[MaxBeachLength];        /* Sum of local diffusivities; the wave angle effect * the energy term */
float   AbsDiffSum[MaxBeachLength];     /* Absolute values of DiffSum */
float   Big_Gamma[MaxBeachLength];      /* Array to hold the relative diffusivity values; see Eqn. 6, Ashton & Murray 2006b */
int	    SedTransTimes[MaxBeachLength]; 	/* How many times did sed trans happen here? */
float	QsGross[MaxBeachLength];	    /* Absolute Value of Flux -defined at rt boundary*/
float	QsNet[MaxBeachLength];		    /* Net transport w/ sign - right looking offshore positive- defined at rt boundary*/
float	QsGrad[MaxBeachLength];		    /* gradient in AST - (in - out) difficult to do w/o shadowing, defined at cell*/

/*
   Array variables for the calculated Qs and diffusivity data matched to the cells over the model domain,
   rather than over TotalBeachCells. This is particularly an issue when saving just the ocean-facing
   cells on recurved coasts (those with flying spits) where the diffusivity data may be suspect or
   not calculated. Not an issue for capes and sandwaves. The names are obvious.
   These variables are allocated values in GetDomainIndices(), and used in SaveDiffToFile().

   Added by Chris Thomas
   13/2/15
*/

int     DomainSedTransTimes[Ymax];
float   DomainDiffSum[Ymax];
float   DomainAbsDiffSum[Ymax];
float   DomainBigGamma[Ymax];
float   DomainQsGross[Ymax];
float   DomainQsNet[Ymax];
float   DomainQsGrad[Ymax];
float   DomainShorelineAngle[Ymax];
float   DomainSurroundingAngle[Ymax];
...
void    GetDomainIndices(void);     /* CWT */
...
void	SedTrans(int From, int To, int i, float ShoreAngle, char MaxT); /* LMV and AA's versions combined */
void 	ShadowSweep(void);
void 	TransportSedimentSweep(void);
void    SaveWaveData(void);
...
/* Initialize Variables and Device */
	...
	ShadowXMax = Xmax-5; 	/* Xmax-5 reinstated to be consistent with AA's code*/

/* The standard way of seeding for rand() */
...
/*
   ...
   If the program is initialised to run diffusivity calculations (DoDiff is 1 in CEM_init.dat)
   then:
   DiffStep is initialised to nDiffSteps and used to control the number of diffusivity calculation loops
   nDiffSteps is used, ultimately, to calculate the time-averaged diffusivity in each beach cell
*/
    ...
    if (DoDiff) DiffStep = nDiffSteps;
    printf("Number of diffusivity calculations: %i\n", DiffStep);
    CurrentTimeStep = 0;

/*
   Initialise the variables for diffusivity calculations
*/
...
/*
    If we have been doing diffusivity calculations (DoDiff set to 1) but have reached the end of
    the diffusivity calcs (nDiffSteps = 0) AND StopAfterDiff = 1, then quit the while...
    statement and pass control to the end of the program.
    StopAfterDiff is a flag set in CEM_init.dat
    nDiffSteps is set in CEM_init.dat, but decremented in FindWaveAngle()
*/
        if (StopAfterDiff && DiffStep == 0) break;
/*
    If CurrentTimeStep = DiffCalcStep (set in CEM_init.dat), then DiffPause is set to 'yes' (=1)
    This controls the point at which diffusivity calculations are run over a static
    beach without evolving it, for the duration of nDiffSteps.
    CurrentTimeStep is NOT incremented during the diffusivity calculation phase
    Unless StopAfterDiff = 1, Normal CEM calculations resume once nDiffSteps reaches 0 and
    DiffPause is reset to 'no' (=0); CurrentTimeStep is then incremented in the normal way.
    See FindWaveAngle() for the control over wave angle calculation during diffusivity or
    normal CEM activity.

    Chris Thomas
    Feb '15
*/
        if (!DiffPause) printf("Current time step is: %i\n", CurrentTimeStep);
        if (DoDiff && !DiffPause && CurrentTimeStep == DiffCalcStep)
        {
/*
    Set DiffPause to TRUE, so that beach is not evolved and diff calcs are done
    DiffSteps times
*/
            DiffPause = TRUE;
            printf("DiffPause is TRUE...\n");
            printf("Pausing to do diffusivity calculations...\n");
/*
    Save the current shoreline data at start of diffusivity calculations before carrying on.
    This is the coast line n which the diffusivity calculations will be done
*/
            SaveSandToFile();
            SaveLineToFile();
        }

/* Let user know what phase the program is in */

		if (ChangeWaveClimate == 1 && !DiffPause)
        {
        ...
        else if (DiffPause && DiffStep == nDiffSteps)
        {
            printf("Starting Diffusivity calculations...\n");
        }

        else if (DiffPause && DiffStep == 0)
        {
            DiffPause = FALSE;
            printf("Diffusivity calculations complete...\n");
        }

/*  Time Step iteration - compute same wave angle for Duration time steps */
/*  Calculate Wave Angle */

        if (DiffPause) printf("Diffusivity step : %i\n", DiffStep);
        printf("Getting new wave angle...\n");
		WaveAngle = FindWaveAngle();
/*
    Output wave data. If running diffusivity calculations, SaveWaveData() will save the
    wave angles used for the diffusivity calculations to DiffWaveData.dat
*/
        printf("Saving wave data to file...\n");
        if (Wavedata == 'y') SaveWaveData();

/*
    Loop for Duration at the current wave sign and wave angle
    This loop holds the primary calculations for sed transport
...
/*
    If DiffPause is false then the coastline will be evolved as normal
    If DiffPause is true, then the coastline is not evolved while a
    target wave climate is run over it to calculate stability metrics
*/
            if (!DiffPause)
            {

/* Post U & A information to screen */
			...
			}  // END first <if (!DiffPause)> block

/* Copy 'visible' XMax by YMax data to external model - creates model boundaries */
...
/*
   If not at the point where we do diffusivity calculation...
...
/* Get Out if no good beach spots exist - finish program*/
                ...
                }  // END while (FellOffArray == 'y')

            }

/*
   Lisa Valvo's rock calculation code; ONLY do rock calculations if required;
   otherwise exclude to save computational time
*/

            if (!DiffPause && RockCalcs)
            {
            ...
            if (RockCalcs) RockCalculations();		/*LMV*/

/*
    These core functions are ALWAYS run regardless of whether DoDiff is true or false
    If DoDiff is true then the diffusivity calculation code in SedTrans() is run, otherwise
    it isn't to save computation time
*/
            ShadowSweep();
			DetermineAngles();
...
/*
    These functions are only run if DiffPause is false
*/

            if (!DiffPause)
            {
                DoSink();                   /* clear sand from sink cells */
                CheckOverwashSweep();       /* Check to see if overwash correction necessary */
                FlowInCell();			    /*LMV*/
                FixFlow();			        /*LMV*/
            }
...
/* Only increment CurrentTimeStep if DiffPause is false */

                CurrentTimeStep++;

            }  // End <if (!DiffPause) block

        }  // END <for (xx = 0; xx < Duration; xx++)>

/*
   Decrement DiffStep by one at this point only if DiffPause is true, ready for next loop
   round the diffusivity calculations
*/

        if (DiffPause) DiffStep -= 1;

	}  // END <while (CurrentTimeStep <= StopAfter)>

/* --------------------------------------- END OF MAIN -------------------------------------------------- */

/* Save diffusivity statistics if calculated, for the model domain */

    printf("DoDiff and StopAfterDiff are : %i, %i\n", DoDiff, StopAfterDiff);
    if (DoDiff && StopAfterDiff)
    {
        printf("Sorting out diffusivity statistics...\n");
        GetDomainIndices();
        SaveDiffToFile();
        printf("Exiting CEM after diffusivity calculations...\n");
        printf("Bye...\n");
    }
	else
    {
        printf("Run Complete.  Output file is: %s \n" , Savefilename);
        printf("Exiting after a normal run...\n");
    }
...
/*
   if pausing to do diffusivity calculations over a coastline at a specific time step - e.g.
   at the end of the spin-up phase, then this if-block applies while DiffStep is > 0
   DiffStep is decremented from nDiffSteps to 0 here.
   nDiffSteps, Diff_Asym and Diff_Highness are set in CEM_init.dat
*/
        if (DiffPause)
        {

/* At point where we stop to do diffusivity calculations, set the U and A values */

            if (DiffStep == nDiffSteps)
            {
                Current_Asym = Diff_Asym;
                Current_Highness = Diff_Highness;
            }

/* If we're at the end of the diffusivity calculations, set DiffPause to 0 (false) */

            if (DiffStep == 0) DiffPause = 0;
...
/*
    Or read real wave data in from a data file
*/
...
//    if(ShadowXMax > Xmax) ShadowXMax = Xmax;
...
/* !!! Do not do transport on underneath c'cause it gets all messed up */
...
/*
   Call SedTrans to calculate VolumeIn and VolumeOut
*/
/*
...
*/
			...
			if (DoFlux)
			{
                SedTrans(CalcCell, CalcCell+Next, i, ShoreAngleUsed, MaxTrans);
			}
		}
...
void SedTrans(int From, int To, int i, float ShoreAngle, char MaxT)
...
/* Variables */
    ...
    float   VolAdjust;              /* all the following variables are for diffusivity calculations */
    float   ThisVol = 0;
    float   vv = 0;
    float   cosang = 0;
    float   sinang = 0;
...
(especially with poorly constrained coefficients), so no attempt made
to make this a more perfect imperfection

CWT: Kc is set in the CEM_init.dat file, rather than being hard-coded as in the original

*/

/*
   Do the sediment transport calculations. NOTE: power for WvHeight corrected to 2.4 (12/5)
   See Eqn 9, etc, A&M, 2006b
*/
...
/*
   If DiffPause is 1 then do the diffusivity calculations
   without evolving the shoreline. DiffPause is used to control which functions are active
   in the main loop of the program, so that when DifPause is 1, the coast line is not evolved.
   Chris Thomas
   Added 4/2/15
*/
	if (DiffPause)
	{
/*
   Because VolumeAcrossBorder is an array in this version and AdjustShore is done elsewhere
   in TransportSedimentSweep() in an amendment by Lisa Valvo, ThisVol is used here for the local
   diffusivity statistics calculations in the latter part of SedTrans
*/
        ThisVol = VolumeAcrossBorder[i];
/*
    The VolumeIn/Out not needed here as this version of the code implements Lisa's alternative
    in AdjustShore(). Commented out here for the moment. Probably deletable.
    vv is needed for the diffusivity calculations
*/

		VolAdjust = 1.0/CellWidth/CellWidth/DepthShoreface;
		vv = ThisVol*VolAdjust;

/* want to compute diffusivity here with actual angle */

		cosang = cos(AngleDeep);
		sinang = sin(AngleDeep);

/*
    Equation 10 in Ashton & Murray, 2006b, p. F04012; PsiHere is the diffusivity
    K2/D and T^1/5H^12/5 terms cancel in the calculation of GAMMA (Eqn. 6), in circumstances
    when all these are fixed. Wouldn't be the case for the T and H terms for real wave climates
*/

		PsiHere = Raise(cosang,0.2)*(1.2*(sinang*sinang)-(cosang*cosang));

		if (From > To)
/* Going to the left, flux has negative sign */
		{
			QsGross[To] += vv;
			QsNet[To]-= vv;
			DiffSum[To] += PsiHere;
			AbsDiffSum[To] += fabs(PsiHere);
			SedTransTimes[To]++;
		}
...
/* Going to the right */
		{
			QsGross[From] += vv;
			QsNet[From]+= vv;
			DiffSum[From] += PsiHere;
			AbsDiffSum[From] += fabs(PsiHere);
			SedTransTimes[From]++;
		}

    }

    if (ThisVol < 0) printf("neg vol");
	if (debug6) printf("VolumeAcrossBorder: %f  ", ThisVol);
...
void InitNormal(void)
  ...
  center_CS = InitBeach;
    ...
    /* shoreline has semi-circular bump for diffusivity calcs KMR */
    {
      InitialRock = InitRock;
      ...
      else if (y > ((Ymax/2) + radius))
      {
        InitialBeach = InitBeach;
      }
      else
      {
        InitialBeach = InitBeach + sqrt(Raise(radius,2) - Raise(y - center_AS,2));
      }
    }
...
/*
   If doing diffusivity calculations, we don't want to re-set X[i] and Y[i], otherwise
   DetermineAngles() throws a wobbly...
*/
...
/*
   ...
   if SaveSandFile = 2, then the output is array format, rather than
   a single line. Code included by Andy Barkwith, 2012.
*/
{
	int	x,y;
	char	savename[40];

	printf("\nSaving sand data to file...\n ");
...
/*
   Array output; Andy Barkwith's code
*/
...
/*  Save file name will add extension '.' and the CurrentTimeStep		*/
{
	...
	printf( "Saving shoreline file: %s\n", savename );
	...
	fclose(SaveSandFile);
	printf("Shoreline file saved...\n");

}


void GetDomainIndices(void)
/*
   This function matches the index values in the 1-d arrays of length TotalBeachCells
   holding the calculated diffusivity data, with those of the model domain (Ymax/2 - 3Ymax/2,
   so that only the data in cells orthogonally facing open ocean are saved to the diffusivity
   output file. This function is required specifically for recurved coastlines, where cells
   on the inside of spits are likely to see only few waves and so the diffusivity stats are
   dubious, because of very poor sampling, or simply don't calculate, because they are hidden
   from all waves. Its not a problem for coasts where all cells face the open sea - capes and
   sand waves.

   This function uses Y[] and the prime diffusivity arrays. The output comprises arrays named
   Domain(...) of length Ymax.
...
/* Flag progress...*/
    ...
    printf("Domain_Y starts at : %i\n", Domain_Y);

/* While we're inside alongshore dimensions of the model domain... */

    for (j = 0; j < Ymax; j++)
    {
/*
   Search along the 1-d arrays over TotalBeachCells for the location of the yth model
   domain cell value held in the arrays of the same length; use this value to allocate
   the matching domain diffusivity value, etc, to the Domain... arrays
*/
        printf("Searching along %i TotalBeachCells...\n", (TotalBeachCells-2) );

        for (i = Start_search; i < TotalBeachCells - 2; i++)
        {
            printf("Y[i] is : %i\n", Y[i]);
            if (Y[i] == Domain_Y)
            {

                printf("Found match: Domain_Y = %i; Y[i] = %i; i = %i\n", Domain_Y, Y[i], i);
/*
   Having found a match, then use the index value i to locate the right value in each of the
   1-d arrays; calculate the Big gamma value. This is time-normalised implicitly since the
   DiffSum and AbsDiffSum are identical in the number of steps, so t cancels.
*/
                DomainDiffSum[j] = DiffSum[i];
                DomainAbsDiffSum[j] = AbsDiffSum[i];
                DomainBigGamma[j] = DomainDiffSum[j]/DomainAbsDiffSum[j];
                DomainQsGross[j] = QsGross[i];
                DomainQsNet[j] = QsNet[i];
                DomainQsGrad[j] = QsGrad[i];
                DomainSedTransTimes[j] = SedTransTimes[i];
                DomainSurroundingAngle[j] = SurroundingAngle[i];
                DomainShorelineAngle[j] = ShorelineAngle[i];
/*
   Increment Domain_Y and the point at which we start searching along TotalBeachCells; don't want
   to search from the start of TotalBeachCells each time because this would find all those instances
   where Domain_Y = Y[i] that are on the inside of spits, which we don't want; start searching at
   the next TotalBeachCell, once a match has been found
*/
                Domain_Y += 1;
                Start_search = i + 1;
                break;
...
/*
   Save computed diffusivity stats to a file over the model domain.
   Written originally by Andrew Ashton. This version modified to cope with
   recurved coastlines (spits!)

   Absolute diffusivity (AbsDiffSum) and relative diffusivity (Big_Gamma) added
   February 2015.

   The data are created from the 1-d arrays of data for the whole coastline in
   GetDomainIndices(), hence the different array names
...
printf( "Saving model domain diffusivity data in: %s\n", savediffname );

SaveDiffFile = fopen(savediffname, "w");
...
if (!SaveDiffFile)
{
 ...
 printf("Saving diffusion and Qs stats...\n");

 printf("Saving surrounding coastline angle data...\n");
 fprintf(SaveDiffFile, "\nSurrounding coastline angle data\n");
 for (i = 0; i <= Ymax; i++)
     fprintf(SaveDiffFile, " %f", DomainSurroundingAngle[i]);

 printf("Saving shoreline angle data...\n");
 fprintf(SaveDiffFile, "\nShoreline angle data\n");
 for (i = 0; i <= Ymax; i++)
     fprintf(SaveDiffFile, " %f", DomainShorelineAngle[i]);

 printf("Saving DiffSum data...\n");
 fprintf(SaveDiffFile, "\nSum of the diffusivities\n");
 for (i = 0; i <= Ymax; i++)
     fprintf(SaveDiffFile, " %f", DomainDiffSum[i]);

 printf("Saving Absoulute DiffSum data...\n");
 fprintf(SaveDiffFile, "\nSum of absolute diffusivities\n");
 for (i = 0; i <= Ymax; i++)
     fprintf(SaveDiffFile, " %f", DomainAbsDiffSum[i]);

 printf("Saving relative diffusivity data...\n");
 fprintf(SaveDiffFile, "\nRelative diffusivity data\n");
 for (i = 0; i <= Ymax; i++)
     fprintf(SaveDiffFile, " %f", DomainBigGamma[i]);

 printf("Saving the number of sediment transport events...\n");
 fprintf(SaveDiffFile, "\nSedTransTimes data\n");
 for (i = 0; i <= Ymax; i++)
     fprintf(SaveDiffFile, " %i", DomainSedTransTimes[i]);

 printf("Saving the gross flux data...\n");
 fprintf(SaveDiffFile, "\nGross Flux (QsGross) data\n");
 for (i = 0; i <= Ymax; i++)
     fprintf(SaveDiffFile, " %f", DomainQsGross[i]);

 printf("Saving the net flux data...\n");
 fprintf(SaveDiffFile, "\nNet flux (QsNet) data\n");
 for (i = 0; i <= Ymax; i++)
     fprintf(SaveDiffFile, " %f", DomainQsNet[i]);

 printf("Saving the flux gradient data...\n");
 fprintf(SaveDiffFile, "\nFlux gradient data\n");
 for (i = 0; i <= Ymax; i++)
     fprintf(SaveDiffFile, " %f", DomainQsGrad[i]);

 fclose(SaveDiffFile);
 printf("Diffusion calculation data saved...\n\n");
...
If pausing to undertake diffusivity calculations, the wave data are saved in a separate
file, as they may be taken from a different wave climate to the one being used for the main
run of the program.
...

{
    if (!DiffPause)
    {
        printf("Saving normal wave data...\n");
    ...
    if (DiffPause)
    {
        printf("Saving diffusivity wave data..\n");
/* Open Diff wave data output file stream at the first pass */

        if (DiffStep == nDiffSteps)
        {
            printf("Opening diffusivity wave data file.../n");
            DiffWaveDataFile = fopen(DiffWaveData, "w");
        }

/* Save the diffusivity wave data */

        printf("Wave angle= %lf  Period= %lf Height= %lf \n",WaveAngle, Period, OffShoreWvHt);
        fprintf(DiffWaveDataFile, "%lf %lf %lf\n", (WaveAngle*radtodeg), Period, OffShoreWvHt);
    }
...
/* Input Wave Distribution */
{
	...
	if (Metadata =='y')
    {
        ...
        if (DoDiff)
            {
                fprintf(InitMetaFile, "Run with targeted diffusivity calculations\n");
                fprintf(InitMetaFile, "Paused at %i for diffusivity calculations\n", DiffCalcStep);
                fprintf(InitMetaFile, "Number of steps for diffusivity calculations: %i\n", nDiffSteps);
                if (StopAfterDiff) fprintf(InitMetaFile, "Run stopped after diffusivity calculations\n");
                fprintf(InitMetaFile, "U value for diffusivity calculations: %f\n", (1 - Diff_Highness));
                fprintf(InitMetaFile, "A value for diffusivity calculations: %f\n", Diff_Asym);
            }
        if (RockCalcs) fprintf(InitMetaFile, "Run with rock calculations\n");
        if (!RockCalcs) fprintf(InitMetaFile, "Run without rock calculations\n");
        ...
        if (ChangeWaveClimate == 1)
            {
                fprintf(InitMetaFile, "Wave climate changing through run\n");
